...
Minimal Next.js App Router route handler using the registry `next` middleware pattern with `@amplio/amplio`.

From monorepo root: `pnpm install && pnpm build && pnpm --filter @amplio/example-next-smoke dev` — http://127.0.0.1:3003
