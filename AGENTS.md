...
| Less is more | Tiny runtime (`@useamplio/core`); no sprawling logger surface |
...
amplio/
├── packages/
│   ├── core/          # @useamplio/core — runtime only (defineEvent, init, wide-event lifecycle)
...
├── benchmarks/        # perf + bundle size gates for @useamplio/core
...
Work in the smallest package that owns the change. Do not leak CLI or codegen logic into `@useamplio/core`.

## Public API (frozen surface)

Only these symbols are public from `@useamplio/core`:
...
6. **Dependencies** — prefer peer deps on framework packages; `@useamplio/core` stays dependency-free or near-zero.
...
1. `@useamplio/core` lifecycle (create → set → emit → seal)
...
- **Benchmarks** track `@useamplio/core` bundle size and hot-path `set`/`emit` cost.
