...
1. **`@amplio/amplio`** — immutable runtime for schema-first wide events.
...
```typescript
// telemetry/logger.ts
import { init } from "@amplio/amplio";
...
Auto-emit is middleware responsibility, not `@amplio/amplio` magic.
...
## 11. `@amplio/amplio` internals (non-public)
...
- [x] `@amplio/amplio` exports a frozen public surface (`defineEvent`, `init`, `logger`/`createLogger`, `useLogger`/`runWithLogger`, errors, types) — verified by tests.
...
- [x] `pnpm size` reports `@amplio/amplio` ≤ 8 KB gzip.
...
- [x] User can delete `@amplio/cli` after init and keep editing `telemetry/` with only `@amplio/amplio` installed (documented in README).
