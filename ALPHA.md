...
> **`.event()` on the spine is no longer a footgun** (alpha.13): `getLogger().event(Def)` on an already-named spine now behaves as `.child(Def)` — a separate correlated row, spine preserved — with a dev notice suggesting the explicit spelling. Earlier alphas rebound and sealed the spine, silently losing the `http.request` row. `.child(Def)` remains the canonical spelling. (Older-alpha correlation quirks are in [CHANGELOG.md](./CHANGELOG.md).)
...
> In a stock create-t3-app layout you don't need to do this by hand: `npx @useamplio/cli@alpha init --yes` (or `init --wire`) edits both files below automatically — plus `src/app/api/auth/[...nextauth]/route.ts` when present (see [docs/t3.md](./docs/t3.md) § NextAuth).
...
With `httpBatchLink` / `httpBatchStreamLink`, multiple procedures share one HTTP request. The spine gets `trpc.batched: true`, `trpc.batch_size`, and `trpc.procedures: ["query post.hello", "mutation user.update", …]` — every invocation is counted, including repeat calls to the same procedure. On a batch, `trpc.path` / `trpc.type` are **`null`** (a batch has no single path — group-bys on `trpc.path` never mix single requests with batches); when a procedure errors, the failing procedure lands in `trpc.failed_path` / `trpc.failed_type` instead of overwriting `trpc.path`. In batches, `status` is the **transport** status of the shared HTTP response (often `207` when results are mixed) while `error.*` carries the failing procedure's error (e.g. `error.code: "UNAUTHORIZED"`). For clean per-procedure attribution, emit domain events inside each procedure rather than relying on the spine alone.
...
Files land under `telemetry/…`. Every item carries a `docs` note (printed at the end of the shadcn install) with the wiring step shadcn cannot do — for events that is `amplio doctor --fix` to wire barrel exports.

Browse the hosted items at [/r/](https://amplio-ruddy.vercel.app/r/) (machine-readable: [`/r/registry.json`](https://amplio-ruddy.vercel.app/r/registry.json)). **If shadcn reports "item not found":** only registry items are hosted. Events generated by `amplio add event your.name` are starter schemas created directly in your repo — they never exist on the registry, so `npx shadcn add @useamplio/event-your-name` 404s by design.
...
- `amplio init` — captures the package-manager install output and prints one summary line (`✓ installed @useamplio/amplio, zod (pnpm, 1.2s)`); pass `--verbose` to stream the raw install log. Under `--yes`/`--wire` in a create-t3-app layout it also wraps `src/app/api/auth/[...nextauth]/route.ts` with `withAmplio` (NextAuth `events` callbacks need request scope to correlate), and suggests `amplio add integration …` for detected dependencies (next-auth, better-auth, Clerk, Resend, Polar).
- `amplio paths` — writes the `~telemetry/*` tsconfig path alias (JSONC-comment-safe) and nothing else. Under `init --yes` the alias is applied by default when a tsconfig exists (so wired imports read `~telemetry/middleware/next`, not `../../../../../telemetry/…`); `--no-paths` opts out, and `amplio init --paths` forces it on.
- `amplio smoke <url>` — end-to-end verification: requests the URL, watches `amplio*.jsonl` for a newly emitted row, prints PASS/FAIL with a diagnosis (wrong port / unwired route / init never ran). Requires the JSON sink (`amplio add sink json`); `--timeout <seconds>` adjusts the wait.
- `amplio add integration <id>` — after installing, prints the manual wiring steps (e.g. next-auth: wrap the `[...nextauth]` route + add `events: amplioNextAuthEvents()`) and warns when the integration's target package is missing from `package.json`.
...
- `amplio doctor` — wiring checks (middleware exports, event schemas, tsconfig paths, Turbopack `../logger` import on `telemetry/middleware/next.ts` and `trpc.ts`, `NEXT_RUNTIME` guard in instrumentation.ts, event barrel exports including shadcn-installed events). On create-t3-app layouts it also re-checks the app-side edits init made — `route.ts` still wraps with `withAmplio`, `trpc.ts` still uses `amplioTrpcMiddleware`, the NextAuth route is still wrapped — since those are the edits most likely to be lost in a merge or T3 upgrade.
