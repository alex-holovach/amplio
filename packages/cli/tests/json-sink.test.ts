...
describe("jsonFileSink", () => {
  const prevEnv = process.env.LOGCN_JSON_SINK_PATH;
  ...
  afterEach(() => {
    ...
    if (prevEnv === undefined) {
      delete process.env.LOGCN_JSON_SINK_PATH;
    } else {
      process.env.LOGCN_JSON_SINK_PATH = prevEnv;
    }
    if (root) {
  ...
  it("creates nested parent dirs and appends a JSON line", () => {
    root = mkdtempSync(path.join(tmpdir(), "logcn-json-sink-"));
  ...
  it("uses LOGCN_JSON_SINK_PATH when options.path is omitted", () => {
    root = mkdtempSync(path.join(tmpdir(), "logcn-json-sink-"));
    const filePath = path.join(root, "env", "nested", "from-env.jsonl");

    try {
      process.env.LOGCN_JSON_SINK_PATH = filePath;
  ...
  it("options.path wins over LOGCN_JSON_SINK_PATH when both are set", () => {
    root = mkdtempSync(path.join(tmpdir(), "logcn-json-sink-"));
    const optionsPath = path.join(root, "opts", "from-options.jsonl");
    const envPath = path.join(root, "env", "from-env.jsonl");
    process.env.LOGCN_JSON_SINK_PATH = envPath;
  ...
  it("defaults to logcn.jsonl in cwd when path and env are unset", () => {
    root = mkdtempSync(path.join(tmpdir(), "logcn-json-sink-"));
    delete process.env.LOGCN_JSON_SINK_PATH;
    process.chdir(root);
    const sink = jsonFileSink();
    ...
    const filePath = path.join(root, "logcn.jsonl");
    expect(readFileSync(filePath, "utf8")).toBe(`${JSON.stringify(record)}\n`);
  });
  it("defaults to logcn.jsonl in cwd when LOGCN_JSON_SINK_PATH is empty", () => {
    root = mkdtempSync(path.join(tmpdir(), "logcn-json-sink-"));
    process.env.LOGCN_JSON_SINK_PATH = "";
    process.chdir(root);
    const sink = jsonFileSink();
  ...
  it("defaults to logcn.jsonl in cwd when LOGCN_JSON_SINK_PATH is whitespace-only", () => {
    root = mkdtempSync(path.join(tmpdir(), "logcn-json-sink-"));
    process.env.LOGCN_JSON_SINK_PATH = "   ";
    process.chdir(root);
    const sink = jsonFileSink();
