...
describe("assertValidEventName", () => {
  ...
  it("reports lowercase requirement for uppercase segments", () => {
    expect(() => assertValidEventName("Post.Created")).toThrow(
      'Event ids must be lowercase (got "Post.Created"; try "post.created").',
    );
    expect(() => assertValidEventName("auth.User.signed_up")).toThrow(
      'Event ids must be lowercase (got "auth.User.signed_up"; try "auth.user.signed_up").',
    );
  });

  it("reports the full naming rule for other invalid names", () => {
    expect(() => assertValidEventName("auth")).toThrow(
      /2\+ lowercase dot-separated semantic segments/,
    );
    expect(() => assertValidEventName("auth..user")).toThrow(
      /2\+ lowercase dot-separated semantic segments/,
    );
  });
});
...
describe("eventNameToRelativePath", () => {
  it("maps semantic ids to flat editable Event modules", () => {
    expect(eventNameToRelativePath("post.created")).toBe(
      "events/post-created.ts",
    );
    expect(eventNameToRelativePath("email.sent")).toBe("events/email-sent.ts");
  });

  it("normalizes dots and underscores consistently", () => {
