...
- **Library-first silence:** Use `createLogger()` / `.emit()` without calling `init()` first — `.emit()` returns `null` (event dropped), no sinks run, dev warns once; never throws. Wire `init({ service, env, sinks: [...] })` when you want output. `getConfig()` still throws if called before `init()`.
...
- **`.error(err)`** on `Error` instances: `error.message`, `error.name` (class name), and `error.code` only when `err.code` is a string or number (Node-style `ENOENT`, etc.) — plain `Error` omits `code`.
