...
`@useamplio/cli` is a scaffolder. Once `telemetry/` exists, you can remove the CLI and keep editing events/middleware/sinks with only `@useamplio/core` installed.
...
| **Less is more** | Tiny runtime (`@useamplio/core`); frozen public API |
...
```bash
pnpm build
pnpm --filter @useamplio/core pack
...
Frozen public surface from `@useamplio/core`:
...
**Library-first silence:** Import `@useamplio/core` and call `.set()` / `.emit()` before you wire `telemetry/logger.ts`. Without `init()` and sinks, `.emit()` still returns a record and does not throw — nothing is written anywhere. Call `init()` with at least one sink when you want output. `getConfig()` is stricter: it throws if you call it before `init()`.
...
| Package | Purpose |
|---|---|
| `@useamplio/core` | Runtime: `defineEvent`, `init`, `logger`, wide-event lifecycle |
...
- **`@useamplio/core`** is packable as-is (`dist/` only; optional Zod peer).
...
pnpm typecheck
pnpm registry:build
pnpm size          # @useamplio/core gzip must stay under 8 KB
